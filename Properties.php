...
class Properties
{
     ...
     *
     * @param null|optional $defaults
     * @internal param optional $defaults Properties object to use for defaults.
     ...
     * @return int
     * @internal param reference $string
     */
     ...
     * @param string $line
     * @return bool
     * @internal param string $line reference
     */
    ...
    public function store($file, $header = null)
    {
        ...
        $data .= $this->toString();
        // write
        if (!is_string($file) && get_resource_type($file) === 'file') {
            fwrite($file, $data);
        } else {
     ...
     *
     * @param key
     * @param null|optional $defaultValue
     ...
     *
     * @param bool|optional $everything
     * @return string
     * @internal param optional $everything boolean, default false. If true then comments and blanks will be included.
     */
     ...
     *
     * @param Properties $other
     * @return bool
     * @internal param Properties $other object.
     */
...
class Properties_Section_Comment implements Properties_ISection
{
     ...
     *
     * @param string $value
     * @param optional|string $comment_char
     * @param optional|string $padding
     * @internal param string $value
     * @internal param optional $comment_char comment character, default '#' (allowed values are '#' or !').
     * @internal param optional $padding comment padding string, default ' '.
     * @throw InvalidArgumentException
     */
...
class Properties_Section_Property implements Properties_ISection
{
     ...
     * @param bool $escape_space
     * @return string
     * @internal param character $char
     * @internal param bool $escape_space
     */
...
//        $result = preg_replace('/([\\s=:#!\\\\]|[^\\x20-\\x7e])/e', 'self::_escapeChar("\\1", $escape_space)', $s);
        $callable = function ($escape_space) {
            Properties_Section_Property::_escapeChar("\\1", $escape_space);
        };
        $result = preg_replace_callback('/([\\s=:#!\\\\]|[^\\x20-\\x7e])/', $callable, $s);
    ...
    {
        return self::_testKey($key);
    }

    public static function testValue($value)
    {
        return self::_testValue($value);
    }
     ...
     * @return string
     * @throws Exception
     * @internal param string $s , or array of matches from preg_replace_callback().
     */
     ...
     * Returns the string representation of the section.
     *
     * @param int|unsigned $max_chars_per_line
     * @return string
     * @internal param unsigned $max_chars_per_line integer, maximum number of characters per line (default 120).
     */
